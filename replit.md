...
- **Content Strategy**: Strict adherence to content guidelines, including removal of specific forbidden words and brand names, handled by `scrub-forbidden.mjs`. Extensive internal linking for SEO. Brand-overview hub pages (`/brands` and `/brands/brand-{a..j}`) were removed by `scripts/remove-brand-hubs.mjs` (idempotent — also strips references from sitemap, llms.txt, mega-menu, vercel.json wildcard + deprecated redirects, homepage JSON-LD `significantLink`, and renumbers OS-page BreadcrumbList JSON-LD to positions 1+2). The script also rewrites the previous "Back to the manufacturer overview" link on each of the 40 OS sub-pages to "Back to all pages" (pointing at `/all-pages`). The 40 noindex'd OS sub-pages at `/brands/brand-X/{linux,macos,pc-10,pc-11}` remain on disk for direct-URL access only.
